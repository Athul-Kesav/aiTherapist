...
def main():
    ...
    dataset = dataset.map(format_example, remove_columns=[c for c in dataset.column_names if c not in ["instruction","response"]])

    # --- STEP 2: Tokenization ---
    def tokenize_fn(examples):
        texts = [t for t in examples["text"] if isinstance(t, str)]
        return tokenizer(
            texts,
            truncation=True,          # ensures text longer than max_length is truncated
            padding="max_length",     # ensures all samples have equal length
    ...
    dataset = dataset.filter(lambda x: x is not None and x["input_ids"] is not None)
    ...
    try:
        ...
        print("⚠️ Warning: prepare_model_for_kbit_training failed, continuing anyway:", e)
    ...
    if not available_targets:
        available_targets = ["q_proj", "k_proj", "v_proj", "o_proj", "down_proj", "up_proj"][:2]
